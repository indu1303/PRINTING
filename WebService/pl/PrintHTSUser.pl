...
sub printUser
{
	...
	$duplicateId=($dupId)?$dupId:$duplicateId; 
	    ...
	    ##### print function
           $pId=$cert->printCertificate($userId, $userData, { PRINTER => 1 },$pId,$printerKey,0,13,'','HTS');
...
sub printAccompanyLetter
{
    ...
    my $productId=13;
    my $printerKey ='CA';
    $printId=$cert->printCertificate($userId, $userData, { EMAIL => $userData->{EMAIL} },$printId,$printerKey,$accompanyLetter,$productId,'','HTS');
    return $printId;
}
...
sub printRefaxUser
{
    ...
    my $printerKey='CA';
    if($userData->{COURSE_ID} == 80004 || $userData->{COURSE_ID} == 80008 || $userData->{COURSE_ID} == 80014 || $userData->{COURSE_ID} == 80018){
	    ...
	    my $LACert=Certificate::LosAngeles->new;
  	    $LACert->faxLACertificates(\@toFaxUsers, $faxNum, $courtName, 'HTS', 0 );
	    return 1;
   }else{
	    $userData->{CERTIFICATE_NUMBER}   = $certNumber;
	    ...
	    my $cert = ("Certificate::$certModule")->new;
	    my $pId = 0;
            $pId=$cert->printCertificate($userId, $userData, { FAX => $faxNumber },$pId,$printerKey,0,13,'','HTS');
	    return $pId;
...
sub printMultiSTCUser
{
    ...
    my @printIdArr = $cert->printMultipleCertificate($userId, $userData, $pId, $userId_1, $userData_1, $pId_1, { PRINTER => 1 }, $printerKey, 13,'HTS');
...
sub printLabel
{
    ...
    use Certificate::HTSCertificate;
    my $cert = Certificate::HTSCertificate->new;
