...
sub _generateCertificate
{
    ...
    $self->{PDF}->writeLine( 250-$xDiff, 734,  'for Mature Drivers');
    $self->{PDF}->writeLine( 250-$xDiff, 342,  'for Mature Drivers');

    $self->{PDF}->setFont('HELVETICA', 14);
    $self->{PDF}->writeLine( 190-$xDiff, 718,  'CERTIFICATE OF COMPLETION');
    $self->{PDF}->writeLine( 190-$xDiff, 326,  'CERTIFICATE OF COMPLETION');

	$self->{PDF}->setFont('HELVETICA', 10);

    $self->{PDF}->writeLine( 200-$xDiff, 680, 'This certificate certifies that the named person has successfully completed an');
	$self->{PDF}->writeLine( 200-$xDiff, 667, 'eight-hour Accident Prevention Course for Mature Drivers and is eligible for a');
	$self->{PDF}->writeLine( 200-$xDiff, 655, 'reduction in premium charges.');
    $self->{PDF}->writeLine( 200-$xDiff, 288, 'This certificate certifies that the named person has successfully completed an');
        $self->{PDF}->writeLine( 200-$xDiff, 275, 'eight-hour Accident Prevention Course for Mature Drivers and is eligible for a');
	$self->{PDF}->writeLine( 200-$xDiff, 263, 'reduction in premium charges.');
