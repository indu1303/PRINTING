...
{
    my $self = shift;
    my $product =($self->{PRODUCT})?$self->{PRODUCT}:'TSTG';
...
sub getCompleteUsers
{
	my $self    = shift;
        ...
        if ($constraints->{STATE})
        {
            ...
            $sql->execute($constraints->{STATE},'HOSTED_AFFILIATE','TSTG');
...
select user_id from user_info where print_date >= date_sub(sysdate(),interval 7 day) and course_id in (60002,60010)
EOM
		...
		if(!defined $c || !length $c){
                ###### pull the next id from the appropriate sequence
		                ...
		                my $prefix      = 0;
		                 ...
		                 $self->{PRODUCT_CON}->do('UPDATE tstgo_la_cert_seq SET id=id+1');
                	         my $sId= $self->{PRODUCT_CON}->selectrow_array("SELECT id from tstgo_la_cert_seq");
...
sub getCourseSelection{
    ...
    if(defined $display){
        ...
        $sql->execute($state, $display,'HOSTED_AFFILIATE','TSTG');
    } else {
        ...
        $sql->execute($state,'HOSTED_AFFILIATE','TSTG');
    }
...
sub getCourseDescription {
    ...
    my %tmp;
		...
		$sql->execute($courseId,'HOSTED_AFFILIATE','TSTG');
