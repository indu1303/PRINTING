...
package Certificate::AARPNYClassroom;
...
sub _generateCertificate
{
    ...
    $self->{PDF}->writeLine(140-$xDiff, 686, $flag);
    ...
    $yDisclaimer -= 15;
    my $OFFICECA = $self->{SETTINGS}->getOfficeCa('AARP_CLASSROOM');
    ...
    $self->{PDF}->writeLine( 250-$xDiff, 745,  'AARP Driver Safety');
    $self->{PDF}->writeLine( 250-$xDiff, 353,  'AARP Driver Safety');
    ...
    $self->{PDF}->writeLine( 190-$xDiff, 680, 'This certifies that the person named below has successfully completed a New York');
	$self->{PDF}->writeLine( 190-$xDiff, 667, 'State DMV-approved Motor Vehicle Accident Prevention Course.');
    $self->{PDF}->writeLine( 190-$xDiff, 288, 'This certifies that the person named below has successfully completed a New York');
        $self->{PDF}->writeLine( 190-$xDiff, 275, 'State DMV-approved Motor Vehicle Accident Prevention Course.');
