...
{
    my $self = shift;
    my $product =($self->{PRODUCT})?$self->{PRODUCT}:'CLASS';
...
EOM
        ...
        {
	    my $value = $self->getCookie($v1,'EMAIL_SENT');
	    my $drivingLicense = $self->getUserDriverLicense($v1);
	    if(($drivingLicense =~ m/^DUP/) && ($value eq 1)){
		next;
	    }
...
sub getUserDriverLicense{
   ...
   my $self = shift;
   my ($studentId) = @_;
   my $drivingLicense =$self->{PRODUCT_CON}->selectrow_array('select driving_license from classroom_students where student_id=?',{},$studentId);
...
sub getInstructorEmail{
   ...
   my $self = shift;
   my ($insId) = @_;
   my $email = $self->{PRODUCT_CON}->selectrow_array("select EMAIL from instructor_details where INSTRUCTOR_ID=?",{},$insId);
   return $email;
}

sub getStateByStateId
{
   my $self = shift;
   my ($stateId) = @_;
   return $self->{PRODUCT_CON}->selectrow_array("SELECT SHORT_DEFINITION FROM classroom_state WHERE STATE_ID=?", {}, $stateId);
}
