...
sub getCompleteUsers
{
	...
	##For DSMS, the constraints may not be useful, will be included as and when they are required
    	my $constraintList        = { 
                            ...
                            STATE           => ' CS.course_id in (select cc.course_id from classroom_courses cc ,classroom_state cst where cc.state_id = cst.state_id and cst.short_definition= \'[STATE]\') ',
                                    };
	...
	my $noPrintCourse = $self->{SETTINGS}->{NO_PRINT_COURSE}->{$self->{PRODUCT}};
    	...
    	my $sqlStmt     = <<"EOM";
SELECT DISTINCT CS.STUDENT_ID AS USER_ID FROM class_students CS, ny_classroom NY WHERE NY.CLASS_ID = CS.CLASS_ID AND NY.PRINT_FLAG = '0' AND NY.PAYMENT_FLAG='1' AND CS.COMPLETION_DATE IS NOT NULL AND CS.PRINT_FLAG != 'Y' AND CS.PAYMENT_FLAG=1 [CONSTRAINT]
EOM
...
sub getNextCertificateNumber
{
	my $self = shift;
    	...
    	my $c = $self->{PRODUCT_CON}->selectrow_array("select CERTIFICATE_NUMBER_ISSUED from classroom_certificate_printing where student_id=?",{},$userId);
	if(!$c){
...
sub getUserData
{
    ...
    my $sql         = $self->{PRODUCT_CON}->prepare(<<"EOM");
SELECT CS.STUDENT_ID AS USER_ID, CS.DRIVERS_LICENSE AS DRIVERS_LICENSE, CS.COURSE_ID, DA.SCHOOL_CODE as DA_CODE, DA.LOGIN, date_format(CS.COMPLETION_DATE,'%m/%d/%Y') AS COMPLETION_DATE,date_format(CCP.PRINT_DATE,'%m/%d/%Y') AS PRINT_DATE, CCP.CERTIFICATE_NUMBER_ISSUED,UPPER(CS.FIRST_NAME) as FIRST_NAME, UPPER(CS.LAST_NAME) as LAST_NAME, CS.ADDRESS_1 as ADDRESS_1, CS.ADDRESS_2 as ADDRESS_2, CS.CITY, CS.STATE,CS.ZIP, CS.EMAIL,CC.DS_STATE AS COURSE_STATE,CC.DESCRIPTION as COURSE_AGGREGATE_DESC,'' AS CERT_PROCESSING_ID,'' AS CERT_1,'' AS CERT_2,'' AS CERT_3,'' AS STC_USER_ID,'' AS LOCK_DATE, date_format(CS.DATE_OF_BIRTH,'%m/%d/%Y') AS DATE_OF_BIRTH,CS.PHONE_1 AS PHONE,CL.LOCATION_CODE AS LOCATION_ID,ID.INSTRUCTOR_CODE AS EDUCATOR_ID, ID.INSTRUCTOR_CODE AS INSTRUCTOR_CODE, CONCAT(ID.FIRST_NAME,' ',ID.LAST_NAME) AS INSTRUCTOR_NAME, NC.DELIVERY_ID, NC.AIRBILL_NUMBER, NC.CLASS_ID, DA.DS_SCHOOL_ID AS DS_SCHOOL_ID, DA.SCHOOL_NAME AS DA_SCHOOL_NAME, DA.ADDRESS_1 AS DA_ADDRESS1, DA.ADDRESS_2 AS DA_ADDRESS2,  DA.CITY AS DA_CITY, DA.STATE AS DA_STATE, DA.ZIP AS DA_ZIP, DA.PHONE_1 AS DA_PHONE1 FROM (((((class_students CS left outer join courses CC on CS.COURSE_ID=CC.COURSE_ID)  left outer join ny_classroom NC on CS.CLASS_ID = NC.CLASS_ID) left outer join classroom_certificate_printing CCP on CS.STUDENT_ID=CCP.STUDENT_ID) left outer join locations CL on NC.LOCATION_ID = CL.LOCATION_ID) left outer join driving_school_usr ID on CS.DS_SCHOOL_ID=ID.DS_SCHOOL_ID)  left outer join driving_school DA on CS.DS_SCHOOL_ID=DA.DS_SCHOOL_ID WHERE CS.STUDENT_ID = ? and NC.INSTRUCTOR_ID = ID.LOGIN_ID
EOM
    $sql->execute($userId);
...
sub putUserPrintRecord {
        ...
        my $classId = $self->{PRODUCT_CON}->selectrow_array('select class_id from class_students where student_id = ?', {}, $studentId);
        ...
        if($type eq 'PRINT')
        {
                ...
                $self->{PRODUCT_CON}->do("update class_students set print_flag='Y', certificate_number = '$certNumber', certificate_issue_date=now() where student_id =  $studentId");
    }
	...
	##If all the ceritificates for the class are printed, then put the print_flag
	my $studentCount = $self->{PRODUCT_CON}->selectrow_array("SELECT COUNT(*) FROM class_students WHERE PRINT_FLAG='N' AND CLASS_ID = ?",{},$classId);
	if($studentCount == 0) {
		$self->{PRODUCT_CON}->do("UPDATE ny_classroom SET PRINT_FLAG = '1' WHERE CLASS_ID = $classId");
	}
...
sub _getUserCourseId
{
    ...
    return $self->{PRODUCT_CON}->selectrow_array("select course_id from class_students where student_id = ?",{}, $userId);
}
...
sub getUserInfo {
    my $self=shift;
    my($userId) = @_;
    my $sth =  $self->{PRODUCT_CON}->prepare('SELECT  CS.STUDENT_ID AS USER_ID,CS.DRIVERS_LICENSE as DRIVERS_LICENSE, CCP.PRINT_DATE, CCP.CERTIFICATE_NUMBER_ISSUED as CERTIFICATE_NUMBER FROM class_students CS,classroom_certificate_printing CCP WHERE CS.STUDENT_ID = CCP.STUDENT_ID AND CS.STUDENT_ID = ?');
    $sth->execute($userId);
...
sub getCompletionDays {
    my $self=shift;
    my($userID) =@_;
    return $self->{PRODUCT_CON}->selectrow_array("select to_days(now())-to_days(COMPLETION_DATE) from class_students where STUDENT_ID=123 and COMPLETION_DATE <= now() ",{},$userID);
}
...
sub getUserDriverLicense {
   my $self = shift;
   my ($studentId) = @_;
   my $drivingLicense =$self->{PRODUCT_CON}->selectrow_array('select drivers_license from class_students where student_id=?',{},$studentId);
...
sub getClassStudents {
    ...
    my ($classId) = @_;
    my $sql;
    $sql = $self->{PRODUCT_CON}->prepare("select student_id, concat(first_name,' ',last_name) as name from class_students where class_id = ?");
    $sql->execute($classId);
    my (%tmp, $key, $def,);
    while(($key, $def) = $sql->fetchrow)
    {
...
sub getInstructorCode {
    my $self=shift;
    my($studentId) = @_;	
    return $self->{PRODUCT_CON}->selectrow_array("select dsu.instructor_code from driving_school_usr dsu, class_students cs, ny_classroom nc where dsu.ds_school_id = cs.ds_school_id and dsu.ds_school_id = nc.ds_school_id and nc.class_id = cs.class_id and nc.instructor_id = dsu.login_id and cs.student_id = ?", {}, $studentId);
...
sub getUserContactData
{
    ...
    my $sql         = $self->{PRODUCT_CON}->prepare(<<"EOM");
select STUDENT_ID, DRIVERS_LICENSE, PHONE_1, EMAIL, ADDRESS_1, ADDRESS_2, CITY, STATE, ZIP, FIRST_NAME, LAST_NAME, SEX, date_format(DATE_OF_BIRTH,'%m/%d/%Y') as DATE_OF_BIRTH from class_students where student_id = ?
EOM
...
sub getNYClassStudentsCount {
    my $self=shift;
    my($classId) = @_;
    return $self->{PRODUCT_CON}->selectrow_array("select count(*) from class_students where class_id = ? ",{},$classId);
}
...
sub getClassInfo {
    my $self=shift;
    my($classId) = @_;
    my $sth =  $self->{PRODUCT_CON}->prepare('SELECT CLASS_NAME, INSTRUCTOR_ID, LOCATION_ID, DS_SCHOOL_ID FROM ny_classroom WHERE CLASS_ID = ?');
    $sth->execute($classId);
    my $tmpHash = $sth->fetchrow_hashref;
    $sth->finish;
    return $tmpHash;
