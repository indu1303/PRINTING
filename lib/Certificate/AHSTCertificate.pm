...
package Certificate::AHSTCertificate;
...
sub _generateCertificate
{
    ...
    my $OFFICECA = $self->{SETTINGS}->getOfficeCa('AHST_PO');
    ...
    if (!$faxEmail)
    {
        $self->_printCorporateAddress(60-$xDiff, 690, $OFFICECA,'support@AffordableHomeStudy.com');
        $self->_printCorporateAddress(60-$xDiff, 89, $OFFICECA,'support@AffordableHomeStudy.com');
    }
    ...
    if($countyId && exists $self->{SETTINGS}->{CTSI_COUNTY}->{$countyId}){
        ...
        $self->{PDF}->writeLine( 430-$xDiff, 460, "      Steve Krasnow" );
        $self->{PDF}->writeLine( 275-$xDiff, 440, "CTSI #2002 - L.A. Superior Court TVS #10022 - CCS Approved - NTSA Approved" );
    }
    my $y = 700;
    my $xOffset = 300;
    my $yPS=467;
...
sub _generateMultipleCertificate
{
    ...
    {
        $OFFICECA = $self->{SETTINGS}->getOfficeCa('AHST',1);
    }
...
sub constructor
{
	...
	###### let's create our certificate pdf object
        if($faxEmail){
                $self->{PS} = $self->getFile($self->{SETTINGS}->{TEMPLATESPATH}."/printing/ahstUserCert.ps");
        }
...
sub printRegularLabel
{
    ...
    my $top = $self->{SETTINGS}->{TEMPLATESPATH}."/printing/AHST_Certificate_Label.pdf";
    my $full=1;
    my $bottom='';
    $self->{PDF}->setTemplate($top,$bottom,$full);
    ###### as we do w/ all things, let's start at the top.  Print the header
    ...
    my $productId=12;  ##### This is for AHST
