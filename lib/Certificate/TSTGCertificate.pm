...
package Certificate::TSTGCertificate;
...
sub _generateCertificate
{
    ...
    if($countyId && exists $self->{SETTINGS}->{CTSI_COUNTY}->{$countyId}){
        ...
        $self->{PDF}->writeLine( 430-$xDiff, 460, "Craig Buck - President" );
        $self->{PDF}->writeLine( 275-$xDiff, 440, "CTSI #2020 - L.A. Superior Court TVS #10014 - CCS Approved - NTSA Approved" );
    }
    my $y = 700;
    my $xOffset = 300;
    my $yPS=467;
...
sub constructor
{
	...
	###### let's create our certificate pdf object
        if($faxEmail){
                $self->{PS} = $self->getFile($self->{SETTINGS}->{TEMPLATESPATH}."/printing/tstgUserCert.ps");
        }
