...
sub _generateCertificate
{
    ...
    ###### print the signature
    if(!$faxEmail){
	if($userData->{COURSE_STATE} && ($userData->{COURSE_STATE} eq 'AK' || $userData->{COURSE_STATE} eq 'TX')){
		...
		{
			$self->_printCorporateAddress(400-$xDiff,89, $OFFICECA,'www.DriversEd.com');
		}
		else
		{
    ...
    {	
    	...
    	$self->{PDF}->writeLine ( 35-$xDiff, 25, "VALID FOR COURT USE ONLY FOR NON-COMMERCIAL DRIVERS. YOU MUST SHOW YOUR DRIVER'S LICENSE OR PROVIDE A DRIVING RECORD AT TIME OF SUBMITTAL TO THE COURT." );
    }
...
{
	my $self = shift;
	my ($userId,$top,$bottom,$faxEmail,$residentState,$county,$productId,$ncUser, $scUser)=@_;
