...
{
    my $self = shift;
    my $product =($self->{PRODUCT})?$self->{PRODUCT}:'HTS';
...
select user_id from user_info where print_date >= date_sub(sysdate(),interval 7 day) and course_id in (80004,80008,80014,80018)
EOM
...
sub getNextCertificateNumber
{
	...
	if ($cId == 80004 || $cId == 80008 || $cId == 80014 || $cId == 80018)
	{
...
sub getCTSICourtNumber {
	my $self=shift;
	my ($idsRegulatorId) = @_;
	my $sql = "SELECT CTSI_COUNTY_ID FROM cts_county_map WHERE HTS_REGULATOR_ID = ?";
