...
package Certificate::CAFleetCertificate;
...
sub _generateCertificate
{
    ...
    my $OFFICECA = $self->{SETTINGS}->getOfficeCa('FLEET_CA');
    ...
    if(!$faxEmail){
	    ...
	    $self->_printSignature($yDisclaimer, $layout->{SIGNATURE});
            $self->_printCorporateAddress(60-$xDiff,686, $OFFICECA,'fleet.idrivesafely.ca');
            $self->_printCorporateAddress(60-$xDiff,89, $OFFICECA,'fleet.idrivesafely.ca');
    }
    if ($faxEmail && $faxEmail==1)
    {
            $self->_printCorporateAddress(54-$xDiff,686, $OFFICECA,'fleet.idrivesafely.ca');
            $self->{PDF}->setFont('HELVETICA', 9);
            $self->{PDF}->writeLine(52-$xDiff, 98, "I DRIVE SAFELY");
            $self->_printCorporateAddress(52-$xDiff,89, $OFFICECA,'fleet.idrivesafely.ca');
    }
...
sub printCoverSheet
{
    ...
    $self->_printCorporateAddress(21-$xDiff,662, $OFFICECA,'fleet.idrivesafely.ca');
    ...
    my $productId=37;  ##### This is for Fleet
	...
	###### let's create our certificate pdf object
	 if($faxEmail){
		...
		$self->{PDF} = Certificate::PDF->new($userId,'','','','','',612,792);
                $self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/certfinal_fleet.jpg",
