...
{
    my $self = shift;
    my $product =($self->{PRODUCT})?$self->{PRODUCT}:'FLEET';
...
EOM
	...
	my $bundleUserCheck = $self->{PRODUCT_CON}->selectrow_array("select count(*) from trainee_bundle_map where trainee_id = ?", {}, $userId);
        if($bundleUserCheck >= 1) {
		$retval->{BUNDLE_USER} = $userId;
		$retval->{BUNDLE_COURSES_DESC} = $self->getBundleCourseDescription($userId);
        }
...
EOM
	...
	##Blocking the bundle users here
	##If fleet manager opted for a single certificate, then block the non-base users from returning the data
	
	my $bundleTrainee = $self->{PRODUCT_CON}->selectrow_array("select trainee_id from trainee_bundle_map where user_id = ?", {}, $userId);
	if($bundleTrainee) {
		##bundle user, check whether the certificate is single certificate or individual certificate
		my $courseId = $self->{PRODUCT_CON}->selectrow_array("select course_id from user_info where user_id = ?", {}, $userId);
	
		#my $baseCourseId = $self->{PRODUCT_CON}->selectrow_array("select base_course_id from course_bundle_map where actual_course_id = ?", {}, $courseId);
		my $baseCourseId = $self->{PRODUCT_CON}->selectrow_array("select course_id from user_info where user_id = ?", {}, $bundleTrainee);
		my $certificatePrintingCheck = $self->{PRODUCT_CON}->selectrow_array("select certificate_printing from course_bundle_map where base_course_id = ? limit 1", {}, $baseCourseId); 
		if($certificatePrintingCheck eq 'SINGLE') {
			next; #The certificate printing not required for this user.
		}
		#if($certificatePrintingCheck eq 'MULTIPLE') {
			###not required to print the bundle user
			#print STDERR "\n if($userId == $bundleTrainee) { \n";
			#if($userId == $bundleTrainee) {
				#next;
			#}
		#}
	...
	##For bundle course id
	my $courseId = $self->{PRODUCT_CON}->selectrow_array("select course_id from user_info where user_id = ?", {}, $userId);
	my $bundleCourseCheck = $self->{PRODUCT_CON}->selectrow_array("select certificate_printing from course_bundle_map where base_course_id = ? limit 1", {}, $courseId);
	if($bundleCourseCheck && $bundleCourseCheck eq 'MULTIPLE') {
		##the user is base course user and need the certificates in MULTIPLE, can exclude the user
		next;
	}
...
sub putUserPrintRecord {
	...
	##Check for bundle Course
	my $baseUserId = $self->{PRODUCT_CON}->selectrow_array('select trainee_id from trainee_bundle_map where trainee_id = ? limit 1', {}, $userId);
	if($baseUserId) {
		my $traineeIds = $self->{PRODUCT_CON}->selectrow_array('select group_concat(user_id) from trainee_bundle_map where trainee_id = ?', {}, $baseUserId);
		if($traineeIds) {
			##update the data
			my $sth1 = $self->{PRODUCT_CON}->prepare("update user_info set print_date = sysdate(), certificate_number = ? where user_id in($traineeIds)");
			$sth1->execute($certNumber);
		}
	}
...
sub getBundleCourseDescription {
	my $self=shift;
	my ($baseUserId) = @_;
	my $userIds = $self->{PRODUCT_CON}->selectrow_array("select group_concat(user_id) from trainee_bundle_map where trainee_id = ?", {}, $baseUserId);
	my $courseDescriptions = $self->{PRODUCT_CON}->selectrow_array("select group_concat(c.short_desc) from course c, user_info ui where ui.course_id = c.course_id and ui.user_id in($userIds)");
