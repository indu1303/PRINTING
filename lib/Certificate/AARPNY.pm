...
sub _generateCertificate
{
    ...
    $self->{PDF}->writeLine(60-$xDiff, 732, 'AARP Smart Driver');
    $self->{PDF}->writeLine(60-$xDiff, 722, "Online Course");
    $self->{PDF}->writeLine(60-$xDiff, 340, 'AARP Smart Driver');
    $self->{PDF}->writeLine(60-$xDiff, 330, 'Online Course');


    ###### print the signature
    #if(!$faxEmail){
	$self->_printVertCorporateAddress(60-$xDiff,712, $OFFICECA,'www.aarpdriversafety.org');
	$self->_printVertCorporateAddress(60-$xDiff,320, $OFFICECA,'www.aarpdriversafety.org');
    ...
    $self->{PDF}->writeLine( 195-$xDiff, 680, 'This certifies that the person named below has successfully completed a New York');
	$self->{PDF}->writeLine( 195-$xDiff, 667, 'State DMV-approved Internet Point and Insurance Reduction Program.');
    $self->{PDF}->writeLine( 195-$xDiff, 288, 'This certifies that the person named below has successfully completed a New York');
        $self->{PDF}->writeLine( 195-$xDiff, 275, 'State DMV-approved Internet Point and Insurance Reduction Program.');
	...
	$self->{PDF}->setFont('HELVETICABOLD', 12);
    	$self->{PDF}->writeLine( 470-$xDiff, 420, $userData->{CERT_1});
        $self->{PDF}->setFont('HELVETICA', 10);
	$self->{PDF}->writeLine(60, 420, 'AARP | 780 3rd Ave. FL 33 | New York, NY 10017-2024');
    }	
	$self->{PDF}->writeLine(60, 40, 'AARP | 780 3rd Ave. FL 33 | New York, NY 10017-2024');
	$self->{PDF}->setFont('HELVETICABOLD', 12);
