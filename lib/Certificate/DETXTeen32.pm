...
package Certificate::DETXTeen32;
...
#	$yDiff='396';
    $self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/DETXTeen32.jpg",
...
#        $self->{PDF}->writeLine( 270-$xDiff, 634-$yDiff, $userData->{COMPLETION_DATE});
	...
	if($userData->{INSTRUCTORDATA}->{OLD_INSTRUCTOR_DATA}){
        	...
        	$self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/". $self->{SETTINGS}->{DIRECTOR_OF_SCHOOL_SIGNATURE_NAME_DE}.".jpg",
                              130-$xDiff, 571-$yDiff, 42, 14,1050,305);
	}
        $self->{PDF}->writeLine( 300-$xDiff, 597-$yDiff, $userData->{INSTRUCTORDATA}->{TEANUMBER});
        $self->{PDF}->writeLine( 300-$xDiff, 597-$yDiff, ''); ########## License Number
...
#      }
    ...
    $self->{PDF}->getCertificate;
     ...
     $self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/DETXTeen32Transfer.jpg",	 
...
#   }
	...
	$self->{PDF}->writeLine(328-$xDiff, 689-$yDiff, "$schoolData1->{ADDRESS_1},$schoolData1->{CITY},$schoolData1->{STATE} $schoolData1->{ZIP} H5" );	 
    ...
    $self->{PDF} = Certificate::PDF->new($userId,'','','','','',612,792);
    $yDiff=-20;
    $self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/DETXTeen32LearnerPermit.jpg",
	    ...
	    if($userData->{REPLACED_CERTIFICATE_NUMBER}){
                ...
                $self->{PDF}->writeLine(470, 322+$yDiff, $userData->{REPLACED_CERTIFICATE_NUMBER} );
            }
	...
	if($userData->{INSTRUCTORDATA}->{OLD_INSTRUCTOR_DATA}){
		...
		$self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/". $self->{SETTINGS}->{DIRECTOR_OF_SCHOOL_SIGNATURE_NAME}.".jpg",
                              114, 114+$yDiff, 42, 14,1050,305);
		$self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/". $self->{SETTINGS}->{DIRECTOR_OF_SCHOOL_SIGNATURE_NAME_DE}.".jpg",
                              114, 138+$yDiff, 42, 14,1050,305);
	}
        #$self->{PDF}->writeLine( 300, 138+$yDiff, $userData->{INSTRUCTORDATA}->{TEANUMBER});
	$self->{PDF}->writeLine( 300, 138+$yDiff, '4433'); ########## License Number
	$self->{PDF}->writeLine( 470, 150+$yDiff, 'Easy Driving School LLC');
	$self->{PDF}->writeLine( 470, 138+$yDiff, 'dba Driversed.com');
 	$self->{PDF}->writeLine( 300, 116+$yDiff, 'C2548');
	if($userData->{PARTIAL_TRANSFER}){
    ...
    if(!$userData->{PARTIAL_TRANSFER}){
	#print STDERR "\n Not Partial transfer : /usr/bin/lp -o nobanner -q 1 -d $printer  -o media=$media $outputFile \n";
	...
	#print STDERR "\n If Partial transfer : /usr/bin/lp -o nobanner -q 1 -d $printer  -o media=$media $outputFile \n";
	    close $ph;
    }
    if(-e $outputFile){
        unlink $outputFile;
...
sub printCertificate
{
    my $self = shift;
    my ($userId,$userData,$outputType,$printId,$printerKey,$accompanyLetter,$productId,$rePrintData)=@_;
	if($userData->{CERTIFICATETYPE} && $userData->{CERTIFICATETYPE} eq 'PARTCERT') {
	return $self->_generate6HRPermitCertificate($userId,$userData,$outputType,$printId,$printerKey,$accompanyLetter,$productId,$rePrintData);
	}
    ##### ok, let's load up the @args array w/ the params to send into the
    ##### print function
    my $xDiff=0;
	...
	if($userData->{INSTRUCTORDATABYCOMPDATE}->{INSTRUCTORID}){
		...
		if($userData->{INSTRUCTORDATA}->{OLD_INSTRUCTOR_DATA}){
			...
			$self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/". $self->{SETTINGS}->{DIRECTOR_OF_SCHOOL_SIGNATURE_NAME_DE}.".jpg",
                	              130-$xDiff, 595-$yDiff, 42, 14,1050,305);
			$self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/". $self->{SETTINGS}->{DIRECTOR_OF_SCHOOL_SIGNATURE_NAME}.".jpg",
                	              130-$xDiff, 571-$yDiff, 42, 14,1050,305);
		}
		$self->{PDF}->writeLine( 300-$xDiff, 597-$yDiff, $userData->{INSTRUCTORDATA}->{TEANUMBER});
	...
	$self->{PDF}->writeLine( 270-$xDiff, 634-$yDiff, $userData->{COMPLETION_DATE});
		...
		if($userData->{COURSE_REASON}) {
                        if($userData->{COURSE_REASON} eq 'DEDS') {
                                ...
                                $self->{PDF}->writeLine(177-$xDiff, 722-$yDiff, 'X' );## Transfer check, except for DEDS
                        }
                }
...
#############Next Page##########################333
	...
	if($userData->{INSTRUCTORDATA}->{OLD_INSTRUCTOR_DATA}){
		...
		$self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/". $self->{SETTINGS}->{DIRECTOR_OF_SCHOOL_SIGNATURE_NAME_DE}.".jpg",
                              114-$xDiff, 140-$yDiff, 42, 14,1050,305);
                $self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/". $self->{SETTINGS}->{DIRECTOR_OF_SCHOOL_SIGNATURE_NAME}.".jpg",
                                      114-$xDiff, 114-$yDiff, 42, 14,1050,305);
	}
	...
	$self->{PDF}->writeLine(320, 495, 'C2548' );
	}
...
sub _generateNoticeOfCancellation
{
    ...
    $self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/TXTeen32CancellationNotice.jpg",
	    ...
	    if($userData->{REPLACED_CERTIFICATE_NUMBER}){
    		...
    		$self->{PDF}->writeLine(474-$xDiff, 700-$yDiff, $userData->{REPLACED_CERTIFICATE_NUMBER} );
    	    }
	    ...
	    $self->{PDF}->setFont('HELVETICA', 10);
	$self->{PDF}->writeLine( 130-$xDiff, 487-$yDiff, 'Easy Driving School LLC' );
	$self->{PDF}->writeLine( 130-$xDiff, 475-$yDiff, 'dba Driversed.com' );
    ...
    close $ph;
    if(-e $outputFile){
	    #print STDERR "\n_generateNoticeOfCancellation :: /usr/bin/lp -o nobanner -q 1 -d $printer -o media=$media  $outputFile \n";
	    unlink $outputFile;
    }
    return $printId;
    #return ($self->{PDF},$printId);
...
sub printRegularLabel
{
    ...
    my $outputFile = "/tmp/LABEL$userId.pdf";
      ...
      if($userData->{DELIVERY_ID} eq '1' || $userData->{DELIVERY_ID} eq '5') {
    	my $ph;
	open ($ph,  "| /usr/bin/lp -o nobanner -q 1 -d $printer -o media=$media  $outputFile");
	close $ph;
	if(-e $outputFile){
...
sub printISRFootPrint
{
    ...
    if($pdf){
		...
		close $ph;
	    	unlink $outputFile;
		#print STDERR "\nprintISRFootPrint::  /usr/bin/lp -o nobanner -q 1 -d $printer -o media=$media  $outputFile \n";
	 }
	
   }
}

sub printTXTeenStudentLog {
	my $self = shift;
	my ($userId, $userData) = @_;

	my $requestId = $userData->{PRINTING_REQUEST_ID};
	my $pdfFile = $self->Printing::DriversEd::getTXTeen32HourLog($userId, $userData);
	...
	#print STDERR "\n Print the Attenance sheet: $outputFile  \n";
}
