...
sub printNonUserFedexLabelAARP
{
    ...
    my %data = ( NAME => $name, ATTENTION => $attention, ADDRESS => $address, CITY => $city,
                 STATE => $state, ZIP => $zip, PHONE => $phone, DESCRIPTION => $description,
                 SIGNATURE => $signature, DELIVERY_ID => $deliveryId, PRINTCATEGORYID => $printCategory,PRINTING_STATE=>$state,PRINTING_TYPE=>'CERTFEDX',USERID => $userId, PRINTMANUALLABLE => $printingState, AARPUSER => 1 );
...
sub printNonUserFedexLabelDriversEd
{
    ...
    my %data = ( NAME => $name, ATTENTION => $attention, ADDRESS => $address, CITY => $city,
                 STATE => $state, ZIP => $zip, PHONE => $phone, DESCRIPTION => $description,
                 SIGNATURE => $signature, DELIVERY_ID => $deliveryId, PRINTCATEGORYID => $printCategory,PRINTING_STATE=>$printingState,PRINTING_TYPE=>$printingType,USERID => $userId, PRODUCT_ID => 'DRIVERSED', PRINTMANUALLABLE => $printingState, DRIVERSEDUSER => 1, );
