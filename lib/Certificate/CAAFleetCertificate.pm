...
package Certificate::CAAFleetCertificate;
...
sub _generateCertificate
{
    ...
    if(!$faxEmail){
    	...
    	$self->_printSignature($yDisclaimer, $layout->{SIGNATURE});
       	$self->_printCorporateAddress(60-$xDiff,686, $OFFICECA,'caafleet.idrivesafely.com');
        $self->_printCorporateAddress(409-$xDiff,89, $OFFICECA,'caafleet.idrivesafely.com');
    }
...
sub printCoverSheet
{
    ...
    $self->_printCorporateAddress(21-$xDiff,662, $OFFICECA,'caafleet.idrivesafely.com');
    ...
    my $productId=16;  ##### This is for CAA Fleet
	...
	###### let's create our certificate pdf object
	 if($faxEmail){
		...
		$self->{PDF} = Certificate::PDF->new($userId,'','','','','',612,792);
                $self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/caafleetcertfinal_caa.jpg",
