...
sub _generateCertificate
{
	...
	if($userData->{COURSE_STATE} && $userData->{COURSE_STATE} eq 'OK') {
		...
		$self->{PDF}->writeLine (290, 570, 'satisfactorily answered the test questions provided to me by I Drive Safely,');
		...
		$self->{PDF}->setFont('HELVETICA', 7);
		$self->{PDF}->writeLine (290, 508, 'I hereby certify that the above name student has successfully completed IDriveSafely.com course');
		$self->{PDF}->writeLine (290, 496, 'based on the information in the I Drive Safely database.');
    ...
    if(!$faxEmail){
	if($userData->{COURSE_STATE} && $userData->{COURSE_STATE} eq 'NV') {
		$self->_printSignature($yDisclaimer, $layout->{SIGNATURE},2);
	}elsif($userData->{COURSE_STATE} && $userData->{COURSE_STATE} eq 'OK') {
		##Just print the signature alone!!! 
		$self->_printSignature(470, 0,1,1);
	} elsif($userData->{COURSE_STATE} && $userData->{COURSE_STATE} eq 'VA') {
		$self->_printSignature($yDisclaimer, $layout->{SIGNATURE},1,1,1);
	} else {
	...
	#}
    #}
    ##District-8 check - https://driversed.atlassian.net/browse/EMAINT-318
    $self->{PDF}->setFont('HELVETICA', 8);
    if($userData->{COURSE_STATE} eq 'VA') {
	if($userData->{VA_DISTRICT8_STUDENT}) {
		my $district8Content = "This student is a District 8 student and has completed the Parent/Teen 90 Minute course as required.";
	    	$self->{PDF}->writeLine(60, 425, $district8Content);
    		$self->{PDF}->writeLine(60, 35, $district8Content);
	}
    }
