...
my $NO_PRINT_USIONLINE_COURSE = { map { $_ => 1 } qw(5003 5004 5005 5006) };
=pod
...
This class encapulates all of the USIONLINE database calls in an OOP manner.  This class will provide
methods to return all users and update USIONLINE users as necessary
...
{
    my $self = shift;
    my $product =($self->{PRODUCT})?$self->{PRODUCT}:'USIONLINE';
    ...
    ##### since we're dealing w/ USIONLINE printing, we need to know what type of job we need
...
getCompleteUsers returns a complete list of completed users who are ready to print. In the USIONLINE class, getCompleteUsers will isolate / query the course based only on USIONLINE courses.  CLASSROOM, FLEET, TEEN, and HOSTED_AFFILIATE courses will not be queried.  This indicates the reduction in the number of courses / users which will eventually have to be fully parsed and possibly eliminiated before we can print.
...
Printing::USIONLINE->getCompleteUsers({ STATE => 'TX' });     

## All San Diego Users  
Printing::USIONLINE->getCompleteUsers({ COUNTY_ID => 36 });

## All California Users 
Printing::USIONLINE->getCompleteUsers({ COURSE_ID => 13 });
...
sub getNextCertificateNumber
{
	...
	elsif (my $courseAlias = $self->{SETTINGS}->getCertPoolCourse($cId))
	{
		...
		if(defined $c && length $c)
		{
			...
			{
				$self->updateCertsStock($self->{SETTINGS}->{CERT_ORDERS_MAP}{'TX_USIONLINE'});
			}
	...
	elsif ($cId == 14 || $cId == 17 || $cId == 27)
	{
		...
		$self->updateCertsStock($self->{SETTINGS}->{CERT_ORDERS_MAP}{'NM_USIONLINE'});
...
sub isPrintableCourse
{
    ...
    my $self    = shift;
    my ($courseId) = @_;
    if (exists $NO_PRINT_USIONLINE_COURSE->{$courseId})
    {
        return 0;
    }
    return 1;
