...
my $NO_PRINT_DIP_COURSE = { map { $_ => 1 } qw(5003 5004 5005 5006 200005 100005 400005) };
my $NO_DL_REGULATORS = { map { $_ => 1 } qw(105863) };
=pod
...
delivery_id, date_format(ui.login_date,'%m/%d/%Y') from user_course_payment ucp 
inner join user_info ui force index(idx_user_info_2) on ucp.user_id = ui.user_id 
inner join user_cert_verification uc on uc.user_id = ui.user_id inner join course c on c.course_id = ui.course_id left outer join user_delivery ud 
on ui.user_id=ud.user_id  where ui.course_id not in ($noCourseList) and ui.completion_date is not null and ui.print_date is null 
and ucp.payment_date is not null and uc.verification_date > date_sub(now(),interval 6 month) [CONSTRAINT] 
...
EOM
	    ...
	    if (exists $NO_DL_REGULATORS->{$v3}) {
		if($v4 && $v4 eq 'T0BEADDED') {
			$v4 = 'N/A';
		}
	    }
...
sub getCACourseEvaluationUsers
{
    my $self = shift;

    my ($stDate,$enDate) = $self->{PRODUCT_CON}->selectrow_array("select date_format(date_sub(sysdate(),interval 7 day),'%Y-%m-%d 00:00:00'), date_format(date_sub(sysdate(),interval 1 day),'%Y-%m-%d 23:59:59')");

    my $sql = $self->{PRODUCT_CON}->prepare(<<"EOM");
select ui.user_id,uevl.q_id,uevl.answer from user_info ui inner join user_contact uc on ui.user_id=uc.user_id inner join user_course_evaluation ucevl on ucevl.user_id=ui.user_id inner join user_evaluation uevl on ui.user_id=uevl.user_id where ucevl.evaluation_date between '$stDate' and '$enDate' and ui.course_id in (33) and ui.completion_date is not null and ui.drivers_license not like 'TEST%' and ui.login NOT LIKE '%TEST%' AND ui.login NOT LIKE '%CONTINUEDED.COM%' AND ui.login NOT LIKE '%IDRIVESAFELY.COM%' AND ui.login NOT LIKE '%ED-VENTURES-ONLINE.COM%' AND ui.login NOT LIKE '%PRADHITA.COM%' order by ui.user_id asc
EOM
...
sub getNextCertificateNumber
{
	...
	if($cId == 2001 || $cId == 2002 || $cId == 2003 || $cId == 2011 || $cId == 2012 || $cId == 2013 || $cId == 2031 || $cId == 2032 || $cId == 2033)
	{
        ...
        elsif ($cId eq '200005' || $cId eq '400005')
        {
        	return "[TO BE ASSIGNED]";
       	} 
	elsif (my $courseAlias = $self->{SETTINGS}->getCertPoolCourse($cId))
	{
        ###### this certificate must be retrieved from the certificate pool.  
        	my $c;
	        if($cId == 1015){

			my $DB=$self->{SETTINGS}->{DBCONNECTION}->{TAKEHOME};
		        my $DBH_TH ||= DBI->connect("dbi:mysql:$DB->{DBNAME}:$DB->{HOST}", $DB->{USER}, $DB->{PASSWORD});
		        if (! $DBH_TH)         { print STDERR "Error Connecting to the database: $DB->{DBNAME} - $DBI::errstr\n"; return 0; }
                	$c = $DBH_TH->selectrow_array('select min(certificate_number) from certificate where course_id = ?',
                                              {},$courseAlias);
			if($DBH_TH){
				$DBH_TH->disconnect;
			}
		}else{
			$c = $self->{PRODUCT_CON}->selectrow_array('select min(certificate_number) from certificate where course_id = ?',
                                                {},$courseAlias);
		}
		...
		if(defined $c && length $c)
		{
		    if($cId == 1015){
		        ...
		        my $DBH_TH ||= DBI->connect("dbi:mysql:$DB->{DBNAME}:$DB->{HOST}", $DB->{USER}, $DB->{PASSWORD});
	                ...
	                if(defined $c && length $c)
        	        {
                	    my $status = $DBH_TH->do('delete from certificate where certificate_number = ? and course_id = ?', {},$c, $courseAlias);
			    if($DBH_TH){
                                $DBH_TH->disconnect;
                            }
			    if(defined $status && $status == 1)
			    { 
				if (exists $self->{SETTINGS}->{ORDERING_COURSE_ITEM_MAPS}{TX}{$cId})
				{
				...
				if (exists $self->{SETTINGS}->{ORDERING_COURSE_ITEM_MAPS}{TX}{$cId})
				{
					$self->updateCertsStock($self->{SETTINGS}->{CERT_ORDERS_MAP}{'TX_DIP'});
				}
				if (exists $self->{SETTINGS}->{ORDERING_COURSE_ITEM_MAPS}{TX_OCPS}{$cId})
				{
					$self->updateCertsStock($self->{SETTINGS}->{CERT_ORDERS_MAP}{'TX_OCPS'});
				}
			    	return $c; 
			    }
		    }
...
EOM
    ...
    my $regulatorId = $self->{PRODUCT_CON}->selectrow_array('select regulator_id from user_info where user_id = ?', {},$userId);
    if (exists $NO_DL_REGULATORS->{$regulatorId} && $retval->{DRIVERS_LICENSE} eq 'T0BEADDED') {
	$retval->{DRIVERS_LICENSE} = 'N/A';
    }
    ...
    my $segmentIdMap = $self->{PRODUCT_CON}->selectrow_array("select value from course_attribute where course_id=? and attribute='SEGMENT_ID'",{},$retval->{COURSE_ID});
    if($segmentIdMap) {
		my $segmentName = $self->{SETTINGS}->{PRODUCT_NAME}->{$segmentIdMap};
		$retval->{SEGMENT_ID_MAP}=$segmentIdMap;
		$retval->{SEGMENT_NAME_MAP}=$segmentName;
    ...
    my $DD_SCHOOL_NUM                 = $self->{PRODUCT_CON}->selectrow_array("select value from user_cookie where user_id = ? and param='SCHOOLNUMC1828'", {},$userId);
    if ($DD_SCHOOL_NUM)
    {
        $retval->{DD_SCHOOL_NUM}      = $DD_SCHOOL_NUM;
    }

   ##DIP NC/SC/KS Project
   my $userCourseState =  $self->{PRODUCT_CON}->selectrow_array("select c.state from course c, user_info ui where ui.course_id = c.course_id and ui.user_id = ?", {}, $userId);
   if($userCourseState eq 'NC') {
   	my $userCouponsCount = $self->{PRODUCT_CON}->selectrow_array("select count(1) from user_coupons where user_id = ?", {}, $userId);
	if($userCouponsCount==0) {
		##Only the signle coupon, get the data
		if($coupon ne 'AAANCC') {
			##Not AAANCC coupon
			$retval->{COURSE_AGGREGATE_DESC} = $retval->{SHORT_DESC};
			$retval->{AAANCCUSER} = 2;
		} else {
			$retval->{AAANCCUSER} = 1;
		}
	} else {
		my $userCoupons = $self->{PRODUCT_CON}->selectrow_array("select group_concat(coupon) from user_coupons where user_id = ?", {}, $userId);
		if($userCoupons) {
			my @couponArr = split(/\,/, $userCoupons);
			my $aaaNcUserCheck = 0;
			foreach my $cpn(@couponArr) {
				if($cpn eq 'AAANCC') {
					$aaaNcUserCheck++;
				}
			}
			if($aaaNcUserCheck && $aaaNcUserCheck >= 1) {
				$retval->{AAANCCUSER} = 1;
			} else {
				$retval->{COURSE_AGGREGATE_DESC} = $retval->{SHORT_DESC};
				$retval->{AAANCCUSER} = 2;
...
EOM
    ...
    my $upsellEmail = $self->{PRODUCT_CON}->selectrow_array("select count(user_id) from user_custom_payment where user_id = ? and (payment_service_id = ? or payment_service_id = ?) and payment_date is not null",{},$userId, $self->{SETTINGS}{UPSELLTYPES}{EMAIL}, $self->{SETTINGS}{UPSELLTYPES}{EMAIL1});
    my $upsellMail = $self->{PRODUCT_CON}->selectrow_array("select count(user_id) from user_custom_payment where user_id = ? and (payment_service_id = ? or payment_service_id = ?) and payment_date is not null",{},$userId, $self->{SETTINGS}{UPSELLTYPES}{MAIL}, $self->{SETTINGS}{UPSELLTYPES}{MAIL1});
...
sub putUserPrintRecord {
    ...
    if($type eq 'PRINT')
    {
	my $userState = $self->{PRODUCT_CON}->selectrow_array("select C.state from user_info UI inner join course C ON UI.course_id=C.course_id where UI.user_id=?",{},$userId);
	...
	if ($userState eq 'TX') {
		my $certprintchk  = $self->{PRODUCT_CON}->selectrow_array("select value from user_cookie where user_id = ? and param='CERTPRINTDATE'", {},$userId);
		my $ucourseId = $self->{PRODUCT_CON}->selectrow_array("select course_id from user_info where user_id=?", {},$userId);
		if ($certprintchk) {
			my $sth =  $self->{PRODUCT_CON}->prepare("update user_cookie set value=concat(NOW(),'~$ucourseId') where user_id = ? and param='CERTPRINTDATE'");		
			$sth->execute($userId);
		} else {
			my $sth =  $self->{PRODUCT_CON}->prepare("insert into user_cookie (user_id, param, value) values (?, ?, concat(NOW(),'~$ucourseId'))");
			$sth->execute($userId,'CERTPRINTDATE');
		}
	}
    }
