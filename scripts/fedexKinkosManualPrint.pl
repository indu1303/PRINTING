...
if(!$opts{R} && !$opts{c} && !$opts{s} && !$opts{A} && !$opts{S} && !$opts{f} && !$opts{m})
{
...
if($opts{m}){
       $users = $API->getManifestUsers($opts{m});
}
...
for my $key(keys %$users)
{
    ...
    }elsif(exists $self->{FEDEXKINKOS}->{$self->{PRODUCT}}->{NONTX}->{$userData->{COURSE_STATE}} && ! $accompanyLetter && !$runSTC & !$affidavit){
                ###### Non TX printing
...
sub TXPrint
{
            ...
            else
            {
                $certsRemaining = $API->getCertificateCount($processOnCourse);
                    $priority++;
                     ...
                     my $certNumber = $uData->{CERTIFICATE_NUMBER};
    ...
    if($delUsers{$user}->{DELIVERY_ID} == 2 ||
		     ...
		     $fedexManifest .= $API->printFedexLabel($user,1,'TX','','',$trackingNumber);
                }
        ...
        foreach my $user(@keys)
        {
            ...
            else
            {
                ...
                if ($certNumber)
                {
                    ...
                    my $result = 0;
                    my $printId = 0;
                    $result=$cert->printCertificate($user, $uData, { PRINTER => 1},$printId,$printerKey,0,$productId,'',$hostedAffRun);
                    if($result)
                    {
                        my $name = "$uData->{FIRST_NAME} $uData->{LAST_NAME}";
                             ...
                             $delUsers{$user}->{DELIVERY_ID} == 11) && !$RUNDUPLICATE && !$accompanyLetter)
                        {
                             my $shippingData = $API->getUserShipping($user);
                             my $trackingNumber = $shippingData->{AIRBILL_NUMBER};
                             $fedexManifest .= $API->printFedexLabel($user,1,$affiliateId,'','',$trackingNumber);
                        }
                    }
                    else
                    {
