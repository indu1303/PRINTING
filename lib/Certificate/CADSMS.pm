...
sub _generateCertificate
{
    ...
    $self->{PDF}->writeLine( 138-$xDiff, 532+$yDiff, $userData->{FIRST_NAME} . ' ' . $userData->{LAST_NAME} );
    $self->{PDF}->writeLine( 367-$xDiff, 532+$yDiff, $userData->{DATE_OF_BIRTH} );


    $self->{PDF}->writeLine( 138-$xDiff, 487+$yDiff, 'I DRIVE SAFELY');
    $self->{PDF}->writeLine( 373-$xDiff, 487+$yDiff, 'E4496');
    #$self->{PDF}->writeLine( 138-$xDiff, 502+$yDiff, $userData->{COMPLETION_DATE});


    $self->{PDF}->writeLine( 138-$xDiff, 464+$yDiff, '283 4th st Unit 301, Oakland, CA 94607');
    $self->{PDF}->writeLine( 370-$xDiff, 463+$yDiff, '866    821 2039');

    #$self->{PDF}->writeLine( 138-$xDiff, 431+$yDiff, $userData->{MAX_SESSION_INSTRUCTOR_NAME});
    #$self->{PDF}->writeLine( 373-$xDiff, 431+$yDiff, $userData->{MAX_SESSION_INSTRUCTOR_DL});

    #my $instructorSignatureFile = $self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/btw/$userData->{MAX_SESSION_INSTRUCTOR}.jpg";
    #if(-e $instructorSignatureFile) {
    #$self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/btw/".$userData->{MAX_SESSION_INSTRUCTOR}.".jpg",
    #                            145-$xDiff, 333+$yDiff, 105, 35,1050,305);
    #} else {
    #$self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/sig.jpg",
    #                            145-$xDiff, 340+$yDiff, 105, 35,1050,305);
    #}
    #$self->{PDF}->writeLine( 373-$xDiff, 340+$yDiff, Settings::getDate);
...
sub printCADSMSLabel
{
    ...
    my $productId=33;  ##### This is for Teen
