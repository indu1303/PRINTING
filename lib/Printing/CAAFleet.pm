...
{
    my $self = shift;
    my $product =($self->{PRODUCT})?$self->{PRODUCT}:'CAAFLEET';
...
#    my $retval;
    ...
    my $sql         = $self->{PRODUCT_CON}->prepare(<<"EOM");
		...
		UC.CITY, UC.STATE, UC.ZIP, UC.EMAIL,
                ...
                FROM
                (((((user_info UI left outer join regulator R on UI.REGULATOR_ID=R.REGULATOR_ID)  left outer join  user_contact UC  on UI.USER_ID=UC.USER_ID)  left outer join course C on UI.COURSE_ID=C.COURSE_ID) left outer join regulator_course_selection RCS on UI.REGULATOR_ID=RCS.REGULATOR_ID)  left outer join user_lockout UL on UI.USER_ID=UL.USER_ID) 
...
EOM
    ...
    while (my ($v1, $v2, $v3) = $sql->fetchrow)
    {
        ...
        #my ($fleet_account_id,$fleet_sponsor)=$self->{PRODUCT_CON}->selectrow_array("select AD.ACCOUNT_ID,AD.ACCOUNT_NAME from user_info UI,account_data AD,trainee_data TCD where UI.USER_ID=TCD.USER_ID AND TCD.ACCOUNT_ID=AD.ACCOUNT_ID and UI.USER_ID=?",{},$userId);
