...
sub _generateCertificate
{
    ...
    if($userData->{FINAL_SCORE}){
	$userData->{FINAL_SCORE} = $userData->{FINAL_SCORE}.'%';
    }
	...
	$OFFICECA = $self->{SETTINGS}->getOfficeCa('AAA_TEEN');
    }else{
	$OFFICECA = $self->{SETTINGS}->getOfficeCa('AAA_TEEN');
    }
	...
	###### print the signature
        ...
        $self->_printCorporateAddress(60-$xDiff,686, $OFFICECA,'howtodriveonline.aaa.com');
        $self->_printCorporateAddress(60-$xDiff,286, $OFFICECA,'howtodriveonline.aaa.com');
    }
    ...
    $self->{PDF}->setFont('HELVETICABOLD', 14);
        $self->{PDF}->writeLine( 138, 770, "AAA How to Drive Online Driver Education - $stateDesc" );
        $self->{PDF}->writeLine( 138, 370, "AAA How to Drive Online Driver Education - $stateDesc" );
