...
sub _generateCertificate
{
    ...
    $self->{PDF}->writeLine(140-$xDiff, 300, $flag);
   ...
   $yDisclaimer = 155;
    ...
    @discMsg=split /~/, $self->getCertificateDisclaimer($layout->{DISCLAIMER});
    foreach my $cc(@discMsg)
    {
        $self->{PDF}->writeLine (350-$xDiff, $yDisclaimer, $cc);
        $yDisclaimer -=12;
    ...
    ###### print the signature
    my $OFFICECA;
    $OFFICECA->{ADDRESS}='Beaver Valley Intermediate Unit #27';
    $OFFICECA->{ADDRESS_2}='147 Poplar Avenue';
    $OFFICECA->{CITY}='Monaca';
    $OFFICECA->{STATE}='PA';
    $OFFICECA->{ZIP}='15061';
    $OFFICECA->{PHONE}='(800) 723-1955';

    if(!$faxEmail){
	$self->{PDF}->writeLine ( 350-$xDiff, 100,'School Signature:_______________________________');
	$self->{PDF}->writeLine ( 350-$xDiff, 500,'School Signature:_______________________________');
	$self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/sigbviu.jpg",
                                398, 500, 149, 34,495,116);
	$self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/sigbviu.jpg",
                                398, 100, 149, 34,495,116);
  	$self->_printCorporateAddress(60-$xDiff,690, $OFFICECA,'teen.idrivesafely.com/bviu');
        $self->_printCorporateAddress(60-$xDiff,290, $OFFICECA,'teen.idrivesafely.com/bviu');
    	$self->{PDF}->setFont('HELVETICA', 9);
  	$self->{PDF}->writeLine(133-$xDiff,701, 'c/o the');
  	$self->{PDF}->writeLine(133-$xDiff,301, 'c/o the');
    }
    ...
    $self->{PDF}->writeLine( 500-$xDiff, 338, $userData->{CERTIFICATE_NUMBER} );
