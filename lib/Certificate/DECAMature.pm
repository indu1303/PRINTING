...
package Certificate::DECAMature;
...
sub _generateCertificate
{
    ...
    $self->{PDF}->writeLine( 280-$xDiff, 451-$yDiff, 'M0334' ,90);
    ...
    $self->{PDF}->writeLine( 328-$xDiff, 80-$yDiff, 'www.DriversEd.com',90);
    $self->{PDF}->writeLine( 328-$xDiff, 260-$yDiff, '283 4th Street, Suite 301,',90);
    $self->{PDF}->writeLine( 328-$xDiff, 451-$yDiff, 'Oakland,CA',90);

    $self->{PDF}->writeLine( 350-$xDiff, 451-$yDiff, Settings::getDate,90);
    $self->{PDF}->writeLine( 350-$xDiff, 210-$yDiff, 'www.DriversEd.com',90);
    $self->{PDF}->writeLine( 403-$xDiff, 80-$yDiff, 'Celia Stokes',90);
    $self->{PDF}->genImage($self->{SETTINGS}->{TEMPLATESPATH}."/printing/images/celia-v-signature.jpg",
...
sub printDECAMatureLabel
{
    my $self = shift;
    my ($userId, $userData) = @_;

    $self->{PDF} = Certificate::PDF->new("LABEL$userId",'','','','','',612,792);
    my $top = $self->{SETTINGS}->{TEMPLATESPATH}."/printing/DE_CA_MATURE_Certificate_Label.pdf";
    my $full=1;
    my $bottom='';
    ...
    $self->_printCorporateAddress(21 , 665, $OFFICECA,'www.driversed.com');
