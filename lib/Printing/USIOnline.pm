...
{
    my $self = shift;
    my $product =($self->{PRODUCT})?$self->{PRODUCT}:'USI_ONLINE';
...
EOM
    ...
    if($$retval{CERTIFICATE_NUMBER} && ($$retval{CERTIFICATE_NUMBER} eq '0' || $$retval{CERTIFICATE_NUMBER} eq '-1')){
        $$retval{CERTIFICATE_NUMBER}=$self->getNextCertificateNumber($userId);
    }
